...
class XR {
  ...
  async enter (mainLoop) {
    ...
    this.session.addEventListener('inputsourceschange', e => {
      for (let added of e.added)
        this.controllers = [added.gamepad].concat(this.controllers);
    });
    ...
    this.session.updateRenderState({ baseLayer: layer, depthFar: 10000 });
    ...
    if (!this.gl.getShaderParameter(vshader, this.gl.COMPILE_STATUS))
      console.error(this.gl.getShaderInfoLog(vshader, this.gl.COMPILE_STATUS));
    const fshader = this.gl.createShader(this.gl.FRAGMENT_SHADER);
    this.gl.shaderSource(fshader, FSHADER);
    this.gl.compileShader(fshader);
    if (!this.gl.getShaderParameter(fshader, this.gl.COMPILE_STATUS))
      console.error(this.gl.getShaderInfoLog(fshader, this.gl.COMPILE_STATUS));
  ...
  display2d(contrast) {
    ...
    this.gl.bufferData(
        this.gl.ELEMENT_ARRAY_BUFFER, this.indices, this.gl.STATIC_DRAW);

    this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.vbuffer);
    this.gl.bufferData(
        this.gl.ARRAY_BUFFER, this.coords, this.gl.STATIC_DRAW);
    ...
    this.gl.bufferData(
        this.gl.ARRAY_BUFFER, this.colors, this.gl.STATIC_DRAW);
...
class Magic2 {
  ...
  translate3dTo2d () {
    if (this[_].xr.activated) {
      this[_].xr.draw(
          this[_].data, this[_].parameters, this[_].depth, this[_].palette);
    } else if (this[_].vr) {
  ...
  point3d(x, y, z, r) {
    if (this[_].xr.activated) {
      ...
      c.beginPath();
      c.arc(
          context.offset + x / d * context.scaleX,
          y / d * context.scaleY,
          r * context.scaleY,
          0, 2 * Math.PI);
